...
class NoteRNNLoader(object):
  ...
  def __init__(self, graph, scope, checkpoint_dir, checkpoint_file=None,
               ...
               note_rnn_type='default', checkpoint_scope='rnn_model',att_length=0):
  ...
  def get_zero_state(self):
    ...
    """
    state_tuple= ()
    for i,layer_size in enumerate(self.hparams.rnn_layer_sizes):
      tup = np.zeros((self.batch_size,layer_size))
      state = tf.contrib.rnn.LSTMStateTuple(tup,tup)      
      if (self.hparams.attn_length!=0 and i==0):
        tensor1 = np.zeros((self.batch_size,layer_size))
        tensor2 = np.zeros((self.batch_size,layer_size*self.hparams.attn_length))
  ...
  def get_full_variable_name_dict(self):
    var_dict = dict()
    for var in self.variables():
      tf.logging.info(var)
      inner_name = rl_tuner_ops.get_inner_scope(var.name)
      ...
      with tf.device(lambda op: ''):
        with tf.variable_scope(self.scope):
          ...
          if self.note_rnn_type == 'basic_rnn':
            #NEW
            if(self.hparams.attn_length==0):
              self.cell = events_rnn_graph.make_rnn_cell(
                  self.hparams.rnn_layer_sizes)
            else:
              self.cell = events_rnn_graph.make_rnn_cell(self.hparams.rnn_layer_sizes, attn_length=self.hparams.attn_length)
          else:
          ...
          #NEW
          self.feed_batch_size = tf.placeholder(tf.int32,[])
          self.initial_state = self.cell.zero_state(batch_size=self.feed_batch_size, dtype=tf.float32)
  ...
  def restore_vars_from_checkpoint(self, checkpoint_dir):
    ...
    #NEW
    var_dict_2 = self.get_full_variable_name_dict()
    with self.graph.as_default():
      saver = tf.train.Saver(var_list=var_dict)
      #NEW
      saver_2 = tf.train.Saver(var_list=var_dict_2)
    ...
    #file_name = "/home/miraodasilva/Desktop/wow.ckpt"

    #if self.scope == 'q_network':
    #  saver_2.save(self.session, "/home/miraodasilva/Desktop/wow.ckpt")
