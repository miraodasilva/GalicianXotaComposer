...
C_MAJOR_TONIC = [2,14,26]

A_MINOR_TONIC = [11,23,35]
#NEW
C_MINOR_TONIC = [2,14,26]
C_MINOR_SCALE = [2,4,5,7,9,10,12,14,16,17,19,21,22,24,26]
C_MINOR_KEY = [0,1,2,4,5,7,9,10,12,14,16,17,19,21,22,24,26,28,29,31,33,34,36]

#NEW
def scale_maker(major,add):
  if major:
    scale = C_MAJOR_SCALE
    key = C_MAJOR_KEY
  else:
  ...
  for el in key:
    if(el <=1 ):
      ...
      new_key += [el+add]
  new_tonic = [el+add for el in tonic]
  return (new_scale,new_key,new_tonic)
#END OF NEW


#NEW
C_SHARP_MAJOR_SCALE,C_SHARP_MAJOR_KEY,C_SHARP_MAJOR_TONIC = scale_maker(True,1)
D_MAJOR_SCALE,D_MAJOR_KEY,D_MAJOR_TONIC= scale_maker(True,2)
F_MAJOR_SCALE,F_MAJOR_KEY,F_MAJOR_TONIC= scale_maker(True,5)
D_MINOR_SCALE,D_MINOR_KEY,D_MINOR_TONIC = scale_maker(False,2)
#END OF NEW
...
def decoder(event_list, transpose_amount):
  ...
  """
  #NEW, added -NUM_SPECIAL_EVENTS to else statement
  return [e - NUM_SPECIAL_EVENTS if e < NUM_SPECIAL_EVENTS else
          e + INITIAL_MIDI_VALUE - transpose_amount - NUM_SPECIAL_EVENTS  for e in event_list]

#NEW
#translates a Monophonic Melody into an RL Tuner sequence
def encoder(event_list, transpose_amount):
  return [e + NUM_SPECIAL_EVENTS if e < NUM_SPECIAL_EVENTS else
          e - INITIAL_MIDI_VALUE + transpose_amount + NUM_SPECIAL_EVENTS  for e in event_list]  
#END OF NEW
