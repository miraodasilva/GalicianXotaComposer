...
#SWITICHING THIS TO MATCH THE MELODY RNN ENCODING!!!!
NOTE_OFF = 1
NO_EVENT = 0
#NEW
BAR_LENGTH=12
...
class RLTuner(object):
  ...
  def __init__(self, output_dir,
               ...
               note_rnn_checkpoint_dir="/home/miraodasilva/anaconda2/envs/magenta/lib/python2.7/site-packages/magenta/models/rl_tuner/checkpoints",
               note_rnn_checkpoint_file="model.ckpt-571",
               #OLD
    ...
    with self.graph.as_default():
      ...
      self.scale = scale
      #EXTRA
      #self.eighth_note_reward = eighth_note_reward
      #END OF EXTRA
      ...
      if self.note_rnn_hparams is None:
        if self.note_rnn_type == 'basic_rnn' and self.attention:
          self.note_rnn_hparams = rl_tuner_ops.attention_rnn_hparams()
        elif self.note_rnn_type == 'basic_rnn' and not self.attention:
  ...
  def initialize_internal_models_graph_session(self,
    ...
    with self.graph.as_default():
      ...
      #NEW
      tf.logging.info(self.note_rnn_hparams)
      #END OF NEW
  ...
  def train(self, num_steps=10000, exploration_period=5000, enable_random=True):
    ...
    for i in range(num_steps):
      ...
      new_state = self.q_network.state_value
      new_reward_state = self.reward_rnn.state_value
    ...
    (action, action_softmax, self.q_network.state_value,
     reward_scores, self.reward_rnn.state_value) = self.session.run(
         ...
         {self.q_network.melody_sequence: input_batch,
          ...
          #NEW
          self.q_network.feed_batch_size: self.q_network.batch_size,
          self.reward_rnn.feed_batch_size: self.reward_rnn.batch_size})
  ...
  def training_step(self):
    ...
    if self.num_times_train_called % self.dqn_hparams.train_every_nth == 0:
      ...
      #Batch states NEW
      states= self.create_empty_state(self.q_network.hparams.rnn_layer_sizes,len(samples))
      new_states = self.create_empty_state(self.target_q_network.hparams.rnn_layer_sizes,len(samples))
      reward_new_states = self.create_empty_state(self.reward_rnn.hparams.rnn_layer_sizes,len(samples))
      #print(states)
      #END OF NEW
      ...
      for i, (o, s, a, r, new_o, new_s, reward_s) in enumerate(samples):
        ...
        #OLD
        #states[i, :] = s
        for j in range(len(self.q_network.hparams.rnn_layer_sizes)):
          if(self.attention and j==0):
            states[0][0].c[i,:] = s[0][0].c[0,:]
            states[0][0].h[i,:] = s[0][0].h[0,:]
            states[0][1][i,:] = s[0][1][0,:]
            states[0][2][i,:] = s[0][2][0,:]
          else:
            states[j].c[i,:] = s[j].c[0,:]
            states[j].h[i,:] = s[j].h[0,:]
        
        #OLD
        #new_states[i, :] = new_s
        """for j in range(len(self.target_q_network.hparams.rnn_layer_sizes)):
          new_states[j].c[i,:] = new_s[j].c[0,:]
          new_states[j].h[i,:] = new_s[j].h[0,:]
        """
        for j in range(len(self.q_network.hparams.rnn_layer_sizes)):
          if(self.attention and j==0):
            new_states[0][0].c[i,:] = new_s[0][0].c[0,:]
            new_states[0][0].h[i,:] = new_s[0][0].h[0,:]
            new_states[0][1][i,:] = new_s[0][1][0,:]
            new_states[0][2][i,:] = new_s[0][2][0,:]
          else:
            new_states[j].c[i,:] = new_s[j].c[0,:]
            new_states[j].h[i,:] = new_s[j].h[0,:]
        ...
        """for j in range(len(self.reward_rnn.hparams.rnn_layer_sizes)):
          reward_new_states[j].c[i,:] = reward_s[j].c[0,:]
          reward_new_states[j].h[i,:] = reward_s[j].h[0,:]
        """
        for j in range(len(self.q_network.hparams.rnn_layer_sizes)):
          if(self.attention and j==0):
            reward_new_states[0][0].c[i,:] = reward_s[0][0].c[0,:]
            reward_new_states[0][0].h[i,:] = reward_s[0][0].h[0,:]
            reward_new_states[0][1][i,:] = reward_s[0][1][0,:]
            reward_new_states[0][2][i,:] = reward_s[0][2][0,:]
          else:
            reward_new_states[j].c[i,:] = reward_s[j].c[0,:]
            reward_new_states[j].h[i,:] = reward_s[j].h[0,:]
      ...
      if self.algorithm == 'g':
        _, _, target_vals, summary_str = self.session.run([
            ...
            #NEW
            self.q_network.feed_batch_size : len(samples),
            self.target_q_network.feed_batch_size : len(samples),
            self.reward_rnn.feed_batch_size : len(samples),
        })
      else:
        _, _, target_vals, summary_str = self.session.run([
            self.prediction_error,
  ...
  def collect_reward(self, obs, action, reward_scores):
    ...
    if self.reward_mode == 'scale':
      ...
      reward = self.reward_scale(obs, action,scale=self.scale)
    elif self.reward_mode == 'key':
      # Makes the model play within a key.
      reward = self.reward_key_distribute_prob(action,key=self.key)
    elif self.reward_mode == 'key_and_tonic':
      # Makes the model play within a key, while starting and ending on the
      # tonic note.
      reward = self.reward_key(action,key=self.key)
      reward += self.reward_tonic(action,tonic_note=self.tonic)
      ...
      reward = self.reward_preferred_intervals(action,key=self.key)
      ...
      return reward * self.reward_scaler + note_rnn_reward
    #NEW
    elif self.reward_mode=='music_theory+galician':
      tf.logging.debug('Note RNN reward: %s', note_rnn_reward)
      reward = self.reward_galician_music_theory(action)

      tf.logging.debug('Total galician+music_theory reward: %s',
                       self.reward_scaler * reward)
      tf.logging.debug('Total note rnn reward: %s', note_rnn_reward)

      self.music_theory_reward_last_n += reward * self.reward_scaler
      return reward * self.reward_scaler + note_rnn_reward
    elif self.reward_mode=='galician_only':
      tf.logging.debug('Note RNN reward: %s', note_rnn_reward)
      reward = self.reward_galician_only(action)

      tf.logging.debug('Total galician reward: %s',
                       self.reward_scaler * reward)
      tf.logging.debug('Total note rnn reward: %s', note_rnn_reward)

      self.music_theory_reward_last_n += reward * self.reward_scaler
      return reward * self.reward_scaler + note_rnn_reward
    elif self.reward_mode=='zero_reward':
      return note_rnn_reward
    elif self.reward_mode=='happy':
      tf.logging.debug('Note RNN reward: %s', note_rnn_reward)
      reward = self.reward_happy_music_theory(action)

      tf.logging.debug('Total happy reward: %s',
                       self.reward_scaler * reward)
      tf.logging.debug('Total note rnn reward: %s', note_rnn_reward)

      self.music_theory_reward_last_n += reward * self.reward_scaler
      return reward * self.reward_scaler + note_rnn_reward
    elif self.reward_mode=='sad':
      tf.logging.debug('Note RNN reward: %s', note_rnn_reward)
      reward = self.reward_sad_music_theory(action)

      tf.logging.debug('Total sad reward: %s',
                       self.reward_scaler * reward)
      tf.logging.debug('Total note rnn reward: %s', note_rnn_reward)
  ...
  def reward_music_theory(self, action):
    ...
    Returns:
      Float reward value.
    """
    tf.logging.debug("processing all music theory")
    reward = self.reward_key(action,key=self.key)
  ...
  #NEW
  def reward_galician_music_theory(self, action):
    """Computes cumulative reward for all music theory functions.
    ...
    tf.logging.debug("processing galician music theory")
  ...
  def reward_galician_only(self,action):
    reward = 0.0
    action_note = np.argmax(action)
    
    if self.section=="A":
      reward += self.reward_section_A(action_note,self.key)
    elif self.section=="B":
      reward += self.reward_section_B(action_note,self.key)
      
    reward += self.reward_melody(action_note,self.tonic)
    reward += self.reward_galician_leap(action_note,self.key)
    reward += self.reward_intra_section_similarity(action_note)
    reward += self.reward_last_quarter_note(action_note)
    return reward

  def reward_happy_music_theory(self,action):
    action_note = np.argmax(action)
    tf.logging.debug("processing happy music theory")
    ...
    reward += self.reward_happy_lengths(action_note)
    reward += self.reward_happy_intervals(action_note,self.key)
    return reward

  def reward_sad_music_theory(self,action):
    action_note = np.argmax(action)
    tf.logging.debug("processing galician music theory")
    ...
    reward += self.reward_sad_lengths(action_note)
    reward += self.reward_sad_intervals(action_note,self.key)
    reward += self.reward_pauses(action_note)
    return reward
  ...
  def reward_scale(self, obs, action, scale=None):
    ...
    #Assuming scale is C, must change
    if scale is None:
      scale = rl_tuner_ops.C_MAJOR_SCALE
    #Getting the previous note and the current note
    obs = np.argmax(obs)
    action = np.argmax(action)
    reward = 0
    #if the note continues being played it gets small reward since it technically continues on the same scale
    if action == 1:
      reward += .1
    #I don't understand but it is a very small reward so it's ok
    if action > obs and action < obs + 3:
      reward += .05
    #Rewarding action (current note) for being in the assumed scale  
    if action in scale:
      reward += .01
      if obs in scale:
        action_pos = scale.index(action)
        obs_pos = scale.index(obs)
        #if the previous note was the last on the scale, we are rewarded for having a pause. I'm not sure if this warrants such a heavy reward (is this in Gauldin's book?)
        if obs_pos == len(scale) - 1 and action_pos == 0:
          reward += .8
        #if the two notes are sequential in the context of the scale, heavy reward. Makes sense.
  ...
  def reward_key_distribute_prob(self, action, key=None):
    ...
    Returns:
      Float reward value.
    """
    #Assume key of C. Difference between key and scale is that scale is just 7/8 notes while the key is everything that is in that scale starting from any octave.
    if key is None:
      key = rl_tuner_ops.C_MAJOR_KEY

    reward = 0
    #Just reward anything in the key slightly
    action_note = np.argmax(action)
  ...
  def reward_key(self, action, penalty_amount=-1.0, key=None):
    ...
    reward = 0
    #Penalizes substantially for being off key
    action_note = np.argmax(action)
  ...
  def reward_tonic(self, action, tonic_note=rl_tuner_ops.C_MAJOR_TONIC,
    ...
    #reward tonic at beginning of last bar
    if self.beat == 0 or self.beat == first_note_of_final_bar:
      if action_note in tonic_note:
        return reward_amount
    #reward holding the first note of last bar (why?)
    elif self.beat == first_note_of_final_bar + 1:
      if action_note == NO_EVENT:
        return reward_amount
    #reward holding and pause during the last beats of last bar
    elif self.beat > first_note_of_final_bar + 1:
  ...
  def reward_non_repeating(self, action):
    ...
    #gives small positive reward if nto repeating
    if penalty >= 0:
      return .1
  #NEW
  def detect_repeating_notes(self, action_note, galician=False):
    ...
    #punishing more than 1 pause in a row, which is a big deal. I believe this should be changed
    if action_note == NOTE_OFF and num_repeated > 1:
      return True
    #depending on if it contains breaks or held notes, a varying number of repetitions is tolerated until it is punished.
  ...
  def reward_penalize_repeating(self,
    ...
    #punish if repeating detected. This is basically a proxy function
    if is_repeating:
  ...
  def reward_penalize_autocorrelation(self,
    ...
    Returns:
      Float reward value.
    """
    #Penalizes high correlation between current notes and previous notes in order to encourage variety aka encourage not getting into a cycle
  ...
  def detect_last_motif(self, composition=None, bar_length=8):
    ...
    Returns:
      ...
      composition.
    """
    #detects the last bar and classfies it as a motif (or not) depending on some arbitrary rules. This seems like it could be expanded on but it is good enough.
  ...
  def reward_motif(self, action, reward_amount=3.0):
    ...
    Returns:
      Float reward value.
    """
    #Rewards it for the fact that there was a motif and gives further reward if it was complex (had a lot of unique notes). 
    composition = self.composition + [np.argmax(action)]
  ...
  def detect_repeated_motif(self, action, bar_length=8):
    ...
    Returns:
      ...
      otherwise.
    """
    #Detect if the previous notes INCLUDING THE ACTION form a motif, and if so checks it was repeated in the past. If it is, returns true. 
    composition = self.composition + [np.argmax(action)]
    if len(composition) < bar_length:
      return False, None
  ...
  def reward_repeated_motif(self,
    ...
    Returns:
      Float reward value.
    """
    #Rewards repeated motifs. Extra reward if they are complex
  ...
  def detect_sequential_interval(self, action, key=None):
    ...
    #THE RETURN VALUE FOR "INTERVAL" IS NOT A REWARD. IT IS AN IDENTIFIER FOR THE TYPE OF INTERVAL IT IS, WHICH IS THEN MATCHED WITH A REWARD IN ANOTHER FUNCTION
    ...
    c_major = False
    #REMOVE THE C_MINOR IN THIS IF STATEMENT
    if key is None or key == rl_tuner_ops.C_MAJOR_KEY or key == rl_tuner_ops.C_MINOR_KEY:
      key = rl_tuner_ops.C_MAJOR_KEY
      c_notes = [2, 14, 26]
      g_notes = [9, 21, 33]
      e_notes = [6, 18, 30]
      ...
      fifth_notes = [9, 21, 33]
    #NEW
    elif key == rl_tuner_ops.C_SHARP_MAJOR_KEY:
      key = rl_tuner_ops.C_SHARP_MAJOR_KEY
      c_notes = [3, 15, 27]
      g_notes = [10, 22, 34]
      e_notes = [7, 19, 31]
      c_major = True
      tonic_notes = [3, 15, 27]
      fifth_notes = [10, 22, 34]
    elif key == rl_tuner_ops.D_MAJOR_KEY:
      key = rl_tuner_ops.D_MAJOR_KEY
      c_notes = [4, 16, 28]
      g_notes = [11, 23, 35]
      e_notes = [8, 20, 32]
      c_major = True
      tonic_notes = [4, 16, 28]
      fifth_notes = [11, 23, 35]
  ...
  def reward_preferred_intervals(self, action, scaler=5.0, key=None):
    ...
    #Provide rewards for all that was defined in the previous function
  ...
  def detect_high_unique(self, composition):
    ...
    """
    #Detects if the max pitch is unique
  ...
  def detect_low_unique(self, composition):
    ...
    Returns:
      True if the lowest note was unique, False otherwise.
    """
    #Detects if min pitch is unique, excluding of course the two special events 0 and 1
  ...
  def reward_high_low_unique(self, action, reward_amount=3.0):
    ...
    Returns:
      Float reward value.
    """
    #Only provides this reward at the end
    ...
    reward = 0.0
    #gives substantial reward if these are unique
  ...
  def detect_leap_up_back(self, action, steps_between_leaps=6):
    ...
    """
    #Everything is described in the above comment but this code has a lot to it.
    if not self.composition:
      return 0
  ...
  def simple_galician_rule(self,action):
    reward = 0.0
    key = rl_tuner_ops.C_MAJOR_KEY
    #works
    if len(self.composition)==2 and self.composition[-2]==NO_EVENT and self.composition[-1]==NO_EVENT and action != NO_EVENT :
      #good at 100
      reward += 20.0
      tf.logging.info("rewarded1")
    elif len(self.composition)==46 and action ==NOTE_OFF:
      reward += 100.0
      tf.logging.info("rewarded1.5")
    elif len(self.composition)==47 and self.composition[-1] == NOTE_OFF and action == NO_EVENT:
      reward += 1000.0
      tf.logging.info("rewarded2")
    elif(len(self.composition)>=2):
      #reward [X,1] or [0,0]
      #WORKING up to 96 notes, also works for 288 but it is a bit messy
      #Good at 50
      if(self.composition[-2]!=NO_EVENT and self.composition[-1]==NO_EVENT and action!=NO_EVENT):
        reward += 5.0
      #TRY if self.composition==[] and action_note == 17 reward += 20 
      """
      #WORKING up to 96 notes, also works for 288 but it is a bit messy
      if(self.composition[-1]!=NO_EVENT and action!=NO_EVENT):
        reward += 20.0
      """
    """
    if(len(self.composition)>=1):
      if (len(self.composition)==(SECTION_A_1+1) and self.composition[-1]==NO_EVENT and action==NO_EVENT) or (len(self.composition) == (SECTION_A_2+1 or SECTION_A_3+1 or SECTION_A_4+1) and (self.composition[-1]==NOTE_OFF and action==NO_EVENT)):
        reward += 100 
      elif (len(self.composition)%(SECTION_N_BARS*BAR_LENGTH)==SECTION_N_BARS*BAR_LENGTH-1) and (self.composition[-1]==NOTE_OFF and action==NO_EVENT):
        reward += 100
    prev_notes = self.get_prev_notes(1)
    if len(prev_notes)>0 and prev_notes[-1] in key and action>1 and action in key and abs(prev_notes[-1]-action)>=5:
      reward+=10
    """
    return reward

  def reward_section_A(self,action,key):
    reward = 0.0
    #start with quarter pause
    if(len(self.composition)==4) and ( all(note==NO_EVENT for note in self.composition) and action!=NO_EVENT):
      reward+= 50.0
    #Sixteenth note
    if(self.detect_sixteenth_note(action)):
      reward += 0.0
    #Eighth note
    if self.detect_eighth_note(action):
      reward += 10
    #Quarter note
    if self.detect_quarter_note(action):
      reward += 50.0
    #Thirds
    prev_notes = self.get_prev_notes(1)
    if ( len(prev_notes)==1 and prev_notes[-1] in key and action>1 and action in key and ( abs(prev_notes[-1]-action)== 3 or abs(prev_notes[-1]-action)== 4 )  ):
      reward += 2.0
    return reward

  def reward_section_B(self,action,key):
    reward = 0.0
    #sixteenth note
    if(self.detect_sixteenth_note(action)):
      reward -= 10.0
    ...
    prev_notes = self.get_prev_notes(1)
    # Fourth or more
    if len(prev_notes)==1 and prev_notes[-1] in key and action>1 and action in key and abs(prev_notes[-1]-action)>=5:
      #tf.logging.info("fourth")
      reward += 2.0
    #Starting last bar with quarter note
    if len(self.composition)==BAR_LENGTH*SECTION_N_BARS-BAR_LENGTH+4 and self.composition[-4]!= NO_EVENT and all(note==NO_EVENT for note in self.composition[-3:]) and action!=NO_EVENT:
      reward += 200.0
    #Ending with two quarter note pauses
    if len(self.composition)>=BAR_LENGTH*SECTION_N_BARS-BAR_LENGTH+4 and action == NOTE_OFF:
      reward +=100.0
    if len(self.composition)==BAR_LENGTH*SECTION_N_BARS-BAR_LENGTH+8 and self.composition[-4]== NOTE_OFF and all(note==NO_EVENT for note in self.composition[-3:]) and action!=NO_EVENT:
      reward += 250.0
    if len(self.composition)==BAR_LENGTH*SECTION_N_BARS-1 and self.composition[-3]== NOTE_OFF and all(note==NO_EVENT for note in self.composition[-2:]) and action==NO_EVENT:
      reward += 250.0
    
    return reward

  def detect_quarter_note(self,action_note):
    if(len(self.composition)>=4):
      if(self.composition[-4]!=NO_EVENT and self.composition[-3]==NO_EVENT and self.composition[-2]==NO_EVENT and self.composition[-1]==NO_EVENT and action_note!=NO_EVENT) or \
      (self.composition[-3]!=NO_EVENT and self.composition[-2]==NO_EVENT and self.composition[-1]==NO_EVENT and action_note==NO_EVENT and len(self.composition)==self.num_notes_in_melody-1):
        return True
    return False

  def detect_eighth_note(self,action_note):
    if(len(self.composition)>=2):
      if(self.composition[-2]!=NO_EVENT and self.composition[-1]==NO_EVENT and action_note!=NO_EVENT) or \
      (self.composition[-1]!=NO_EVENT and action_note ==NO_EVENT and len(self.composition)==self.num_notes_in_melody-1):
        return True
    return False

  def detect_sixteenth_note(self,action_note):
    if(len(self.composition)>=1):
      if(self.composition[-1]!= NO_EVENT and action_note != NO_EVENT) or (action_note != NO_EVENT and len(self.composition)==self.num_notes_in_melody-1):
        return True
    return False

    
  def reward_galician_scales(self, action,key): #used to have obs as parameter
    reward = 0.0
    #reward_scale(self,obs,action,scale=scale)
    #reward_key_distribute_prob(self,action,scale=scale)
    reward += self.reward_key(action,key=key)
    return reward

  def reward_last_quarter_note(self,action_note):
    reward = 0.0
    if len(self.composition)==SECTION_N_BARS*BAR_LENGTH-1 and self.composition[-3] != NO_EVENT and all(note == NO_EVENT for note in self.composition[-2:]) and action_note == NO_EVENT:
      reward += 200.0
    return reward

  def reward_intra_section_similarity(self,action):
    #Ensure we are in the second half of the section
    reward = 0.0
    if len(self.composition)%(BAR_LENGTH*SECTION_N_BARS) >= (int(BAR_LENGTH*SECTION_N_BARS/2))+1:
      reward += self.reward_similar_interval(action)
    elif len(self.composition)%(BAR_LENGTH*SECTION_N_BARS)==0 and len(self.composition)>0:
      reward += self.transposition_consistency(action)
    return reward

  def reward_similar_interval(self,action,reward_amount=5.0):
    previous_interval= (self.composition[-int((BAR_LENGTH*SECTION_N_BARS/2))-1],self.composition[-(int(BAR_LENGTH*SECTION_N_BARS/2))])
    current_interval = (self.composition[-1],action)
    #IF they are the exact same
    if(previous_interval==current_interval):
      return reward_amount
    #IF there is pause/held note and then some other note
    elif(previous_interval[0]<=1 and previous_interval[0]==current_interval[0]) and (previous_interval[1]>1 and current_interval[1]>1):
      return reward_amount
    #Vice versa from above
    elif(previous_interval[1]<=1 and previous_interval[1]==current_interval[1]) and (previous_interval[0]>1 and current_interval[0]>1):
      return reward_amount
    #If the interval is the same but transposed
    elif(all( i > 1 for i in previous_interval+current_interval)):
      return reward_amount
    else:
      return 0.0

  def transposition_consistency(self,action,reward_amount=10.0):
    previous_diff = -1
    reward = 0.0
    for i in range(-int(BAR_LENGTH*SECTION_N_BARS/2),0,+1):
      if(self.composition[i]>1 and self.composition[i+int(BAR_LENGTH*SECTION_N_BARS/2)] > 1 ):
        diff = abs(self.composition[i] - self.composition[i+int(BAR_LENGTH*SECTION_N_BARS/2)])
  ...
  def reward_melody(self,action,tonic,reward_amount=5.0):
    reward =0.0
    if self.detect_late_tonic(action,tonic):
      reward += reward_amount*2 
    if self.detect_sub_tonic(action,tonic):
      reward += reward_amount*0.5
    return reward

  def detect_late_tonic(self,action,tonic):
    if (len(self.composition)%(SECTION_N_BARS*BAR_LENGTH)==SECTION_N_BARS*BAR_LENGTH-1):
      prev_notes = self.get_prev_notes(1)
      if(len(prev_notes)>0 and ((prev_notes[0] in tonic and action<2) or action in tonic )):
        return True
    return False

  def detect_sub_tonic(self,action,tonic):
    prev_notes = self.get_prev_notes(1)
    if(len(prev_notes)==0):
      return False
    prev_note = prev_notes[0]
    if prev_note in [el-1 for el in tonic] and action in tonic:
      return True

  def detect_non_sub_tonic(self,action,tonic):
    prev_notes = self.get_prev_notes(1)
    if(len(prev_notes)==0):
      return False
    prev_note = prev_notes[0]
    if prev_note in [el-1 for el in tonic] and action>1 and action not in tonic:
      return True

  def get_prev_notes(self,n):
    #RETURNS N PREVIOUS NOTES IN THE ORDER THEY APPEAR IN THE COMPOSITION
    i= len(self.composition)-1
    prev_notes = []
    count = 0
    while i>=0 and count<n:
      if self.composition[i]>1 :
        prev_notes = [self.composition[i]] + prev_notes
        count += 1
      i -= 1
    return prev_notes

  def reward_galician_intervals(self,action,key):
    prev_notes = self.get_prev_notes(1)
    reward = 0.0
    if (len(prev_notes)==0):
      return reward
    #Making sure there is a previous note and both notes are in key
    if len(prev_notes)>0 and prev_notes[-1] in key and action>1 and action in key:
      #If we are in A and we have a third
      if( ( (len(self.composition) >= SECTION_A_1 and len(self.composition)<SECTION_B_1) or (len(self.composition) >= SECTION_A_3 and len(self.composition)<self.num_notes_in_melody) ) and ( abs(prev_notes[-1]-action)== 3 or abs(prev_notes[-1]-action)== 4   ) ):
        reward += 0.2
      #If we are in B and have an interval larger or equal to a forth
      elif(len(self.composition) >= SECTION_B_1 and len(self.composition)<SECTION_A_3) and abs(prev_notes[-1]-action)>=5:
        reward += 0.2
    return reward

  def reward_galician_leap(self,action_note,key):
    reward = 0.0
    reward += self.reward_opposite_second(action_note,key)
    reward += self.reward_direction_change(action_note)
    return reward

  def reward_opposite_second(self,action_note,key):
    reward = 0.0
    prev_notes = self.get_prev_notes(2)
    if(len(prev_notes)<2):
      return reward
    prev_note=prev_notes[-1]
    #If we have a second in key opposite to the leap we get an extra reward
    if action_note in key and prev_note in key and abs(prev_notes[-1] - prev_notes[-2])>=5 and abs(prev_note - action_note)<=2 and np.sign(prev_notes[-1] - prev_notes[-2]) != 0 and np.sign(action_note - prev_note) != 0 and np.sign(prev_notes[-1] - prev_notes[-2]) != np.sign(action_note - prev_note):
      reward += 5.0
    return reward

  def reward_direction_change(self,action_note):
    reward = 0.0
    prev_notes = self.get_prev_notes(6)
    if(len(prev_notes)<6):
      return reward 
    prev_note = prev_notes[-1]
    direction = np.sign(action_note - prev_note)
    count = 0
    for i in range(2,7):
      if (np.sign(prev_notes[-i+1]-prev_notes[-i]) != direction ) and direction!=0 and np.sign(prev_notes[-i+1]-prev_notes[-i])!=0 :
        count+=1
      else:
        break
    #IF there was a sequence of 4,5 or 6 notes in the same direction and then we changed direction, reward
    if count==3:
      reward += 1.0
    elif count==4:
      reward += 2.5
  ...
  #END OF NEW
  #EXTRA
  def reward_happy_lengths(self,action):
    reward = 0.0
    if(self.detect_sixteenth_note(action)):
      reward += 2.0
    #Eighth note
    if self.detect_eighth_note(action):
      reward += 5.0
    return reward
  def reward_happy_intervals(self,action,key):
    reward = 0.0
    prev_notes = self.get_prev_notes(1)
    if(len(prev_notes)==0):
      return reward
    prev_note = prev_notes[-1]
    interval = abs(prev_note-action)
    major_intervals=[2,4,5,7,9,11]
    for pos,i in enumerate(major_intervals):
      if i == interval and prev_note in key and action in key:
        reward += 3.0 - (pos+1/2.0)
    return reward

  def reward_sad_lengths(self,action):
    reward = 0.0
    if(self.detect_quarter_note(action)):
      reward += 60.0
    #Eighth note
    if self.detect_eighth_note(action):
      reward += 5.0
    return reward
  def reward_sad_intervals(self,action,key):
    reward = 0.0
    prev_notes = self.get_prev_notes(1)
    if(len(prev_notes)==0):
      return reward
    prev_note = prev_notes[-1]
    interval = abs(prev_note-action)
    minor_intervals=[1,3,6,8,10]
    if interval in minor_intervals and prev_note in key and action in key:
      reward += 3.0
    return reward
  def reward_pauses(self,action):
    if action == NOTE_OFF:
      return 1.0
    return 0.0
  #END OF EXTRA
  ...
  def evaluate_music_theory_metrics(self, num_compositions=10000, key=rl_tuner_ops.C_MAJOR_KEY,
  ...
  def create_empty_state(self,layer_sizes,batch_size):
    state_tuple = ()
    for i,layer_size in enumerate(layer_sizes):
        tup = np.empty((batch_size,layer_size))
        state = tf.contrib.rnn.LSTMStateTuple(tup,tup)
        if (self.attention and i==0):
          tensor1 = np.empty((batch_size,layer_size))
          tensor2 = np.empty((batch_size,layer_size*self.note_rnn_hparams.attn_length))
          state_tuple += ((state,tensor1,tensor2,),)
        else:
          state_tuple += (state,)
    return state_tuple
  #END OF NEW  

#tuner = RLTuner("lixo", note_rnn_type="basic_rnn")
#tuner.generate_music_sequence()
